...
from app.schemas.trading_account import TradingAccountConnectionTestRead, TradingAccountQuoteRead
...
class TradingAccountExecutionService:
    ...
    def fetch_quote(self, account_id: int, user_id: int, symbol: str) -> TradingAccountQuoteRead:
        ...
        try:
            adapter.connect()
            quote = adapter.get_quote(symbol.upper())
            self._mark_success(account)
    ...
    def _mark_success(self, account: TradingAccount) -> None:
