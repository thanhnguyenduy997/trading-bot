...
class TradingViewExportService:
    ...
    def _generate_pine_code(
        ...
        *,
        ...
        lines.append("")
        lines.append("focusTradeNo = input.int(1, 'Focus trade number (0 = all trades)', minval=0)")
        lines.append("showFullDetails = input.bool(true, 'Show full details')")
        lines.append("showAllTradeLabels = input.bool(true, 'All mode: show BUY/SELL labels')")
        lines.append("showAllTradeBoxes = input.bool(false, 'All mode: show compact position boxes')")
        lines.append("showAllTradeLevels = input.bool(false, 'All mode: show Entry/SL/TP lines')")
        lines.append("allPositionWidthHours = input.int(4, 'All mode: position width hours', minval=1, maxval=72)")
        ...
        lines.append("            labelText = showFullDetails ? (directionIsBuy ? 'BUY #' : 'SELL #') + str.tostring(i + 1) : (directionIsBuy ? 'B#' : 'S#') + str.tostring(i + 1)")
        lines.append("            tooltipText = 'Trade #' + str.tostring(i + 1) + '\\nDirection: ' + str.upper(direction) + '\\nEntry: ' + str.tostring(entryPrice) + '\\nSL: ' + str.tostring(sl) + '\\nTP1: ' + str.tostring(tp1) + '\\nTP2: ' + str.tostring(tp2) + '\\nOutcome: ' + outcome + '\\nClose: ' + str.tostring(closePrice) + (str.length(reviewReason) > 0 ? ('\\nReview: ' + reviewReason) : '')")
        lines.append("            if showAllTradeLabels and not na(markerTime) and not na(entryPrice)")
        lines.append("                label.new(markerTime, entryPrice, text=labelText, xloc=xloc.bar_time, style=directionIsBuy ? label.style_label_up : label.style_label_down, color=directionIsBuy ? color.new(color.green, 0) : color.new(color.red, 0), textcolor=color.white, size=size.small, tooltip=tooltipText)")
        lines.append("            allWidthMs = allPositionWidthHours * 60 * 60 * 1000")
        lines.append("            allRightTime = markerTime + allWidthMs")
        lines.append("            if showAllTradeBoxes and not na(markerTime) and not na(entryPrice) and not na(sl) and not na(tp1)")
        ...
        lines.append("            label.new(entryTime, entryPrice, text=displayText, xloc=xloc.bar_time, style=style, color=outcomeColor(outcome), textcolor=color.white)")
